...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('extra', nargs='*',help='Extra arguments')
    ...
    def check_status(self):
        ...
        self.stdout.write('Checking RabbitMQ status...')
        ...
        if registered:
            for worker, tasks in registered.items():
                for task in sorted(tasks):
                    if 'users.tasks' in task:
                        self.stdout.write(f' *\t{task}')

    def test_email(self, extra, wait):  # usage
        from users.tasks import send_verification_email

        if not extra:
            raise CommandError('No extra arguments. Set user_id: python manage.py celery_tasks test_email <user_id>')

        user_id = int(extra[0])

        self.stdout.write(f'Sending verification email to {user_id}...')
    ...
    def test_report(self, wait):
        ...
        self.stdout.write(f' Starting weekly report...')
    ...
    def test_cleanup(self, wait):  # usage
        from users.tasks import cleanup_unverified_users

        self.stdout.write(f' Starting cleanup task...')
