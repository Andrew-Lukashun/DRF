...
@shared_task(bind=True, max_retries=3, default_retry_delay=timedelta(minutes=5))
def send_verification_email(self, user_id):
    ...
    try:
        ...
        if not user.email_verification_token:
            logger.info(f'User {user_id} has no verification token')
            return {'status': 'error', 'reason': 'No verification token'}
        ...
        logger.info(f'[Task {self.request.id}] Email sent')

        return {'staus': 'success', 'reason': f'Email sent to user {user.email}'}


    except User.DoesNotExist:
        logger.info(f'[Task {self.request.id}] User does not exist')
        return {'staus': 'error', 'reason': 'User does not exist'}
...
def send_weekly_report(self):
    ...
    for user in users:
        stats = WorkoutSession.objects.filter(user=user, created_at__gte=week_ago).aggregate(
            total=Count('id'),
            competed=Count('id', filter = Q(stats='competed')),
        duration = Sum('duration_minutes'),
        calories = Sum('calories_burned')
        )

        if stats['total'] and stats['total'] > 0:
            completion_rate = (stats['competed'] or 0) / stats['total'] * 100

            message = f'''
            Hello, {user.email}!
            
            Count of trainings: {stats['competed']}
        ...
        try:
            ...
            logger.error(f'[Task {self.request.id}] Exception: {exc}. Failed to send report to {user.email}')

    logger.info(f'[Task {self.request.id}] Finished sending weekly report')
    logger.info(f'[Task {self.request.id}] Send {sent_count} weekly reports')

    return {'staus': 'success', 'sent_count': sent_count, 'total_users': len(users)}
...
def cleanup_unverified_users(self):
    ...
    logger.info(f'[Task {self.request.id}] Cleaning up unverified users')
