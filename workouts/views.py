...
class WorkoutSessionViewSet(viewsets.ModelViewSet):
    ...
    def stats(self, request):
        ...
        completed_workouts = queryset.filter(status=WorkoutSession.Status.COMPLETED).count()
        total_duration = sum(w.duration_minutes or 0 for w in queryset.filter(status=WorkoutSession.Status.COMPLETED))
        total_calories = sum(w.calories_burned or 0 for w in queryset.filter(status=WorkoutSession.Status.COMPLETED))

        # Calculate average rating
        rated_workouts = queryset.filter(rating__isnull=False)
        avg_rating = None
        if rated_workouts.exists():
            avg_rating = round(sum(w.rating for w in rated_workouts) / rated_workouts.count(), 2)
