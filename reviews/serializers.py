...
class ReviewCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if plan and plan.trainer == user:
            raise serializers.ValidationError({"plan": "You cannot review your own training plan."})

        # Check if user already reviewed this plan
        if Review.objects.filter(plan=plan, user=user).exists():
            raise serializers.ValidationError({"plan": "You have already reviewed this plan."})
...
class TrainerReviewCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if trainer == user:
            raise serializers.ValidationError({"trainer": "You cannot review yourself."})

        # Trainer must be an actual trainer
        if trainer and not trainer.is_trainer:
            raise serializers.ValidationError({"trainer": "You can only review trainers."})

        # Check if user already reviewed this trainer
        if TrainerReview.objects.filter(trainer=trainer, user=user).exists():
            raise serializers.ValidationError({"trainer": "You have already reviewed this trainer."})
